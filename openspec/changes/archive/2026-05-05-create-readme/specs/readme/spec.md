...
Framework" pattern), then three badges (npm version, JSR version, MIT license). All
badges SHALL use `style=flat`, `labelColor=000`, and `color=ffd700`.
...
- **THEN** they are taken to `https://www.npmjs.com/package/@grammyjs/testing`
...
1. Installation commands for npm and JSR (Deno)
...
- **THEN** `npm install --save-dev @grammyjs/testing` installs the package

#### Scenario: JSR install command is correct

- **WHEN** a developer copies the JSR command
- **THEN** `npx jsr add --dev @grammyjs/testing` installs the package from JSR
