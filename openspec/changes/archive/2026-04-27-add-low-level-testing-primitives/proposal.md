...
- Error-simulation API on `outgoing`: `failNext`, `failAll`, `respondNext`, `clearOverrides`. Each accepts a real `GrammyError` _or_ a sugar spec `{ code, description }` that the plugin upgrades to a `GrammyError` internally.
