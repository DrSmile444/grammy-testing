...
- **THEN** the npm install command for `@grammyjs/testing` and `grammy` is shown
...
library with that test runner, including any CommonJS/ESM considerations for Jest.
...
#### Scenario: Deno JSR import is shown

- **WHEN** a user reads "With Deno"
- **THEN** the JSR import `jsr:@grammyjs/testing` is shown
...
high-level API, list the exports available only from `@grammyjs/testing/low-level`, and link
to each low-level page.

#### Scenario: Low-level subpath import is documented

- **WHEN** a user reads the Low-Level overview
- **THEN** it shows `import { GenericMockUpdate } from '@grammyjs/testing/low-level'`
...
`@grammyjs/testing/low-level` subpath: `GenericMockUpdate`, `MessagePrivateMockUpdate`,
