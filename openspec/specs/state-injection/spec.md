...
Defines the high-level `state` option for `prepareComposer` and `prepareMiddleware`, allowing tests to pre-populate `ctx.state` without manually wiring `mockState` middleware.
