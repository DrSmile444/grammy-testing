...
The system SHALL support testing `@grammyjs/auto-retry`'s retry loop. When `autoRetry` is installed via `bot.api.config.use()` before `prepareBot` and the test injects a raw rate-limit response using `chats.outgoing.respondNextRaw`, autoRetry SHALL observe the not-ok response, wait the specified `retry_after` duration, and retry the call. The retried call SHALL resolve with the normal synthetic response. The `retry_after` value used in tests SHALL be `0` or a minimal value to avoid wall-clock delays in CI.
